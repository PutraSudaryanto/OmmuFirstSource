...
class ZoneprovinceController extends Controller
{
	...
	public function actionIndex() 
	{
		...
		$setting = OmmuZoneProvince::model()->findByPk(1,array(
		...
		$dataProvider = new CActiveDataProvider('OmmuZoneProvince', array(
			'criteria'=>$criteria,
			'pagination'=>array(
				'pageSize'=>10,
			),
		...
		$this->pageTitle = 'Ommu Zone Provinces';
		$this->pageDescription = $setting->meta_description;
		$this->pageMeta = $setting->meta_keyword;
		$this->render('/zone_province/front_index',array(
	...
	public function actionView($id) 
	{
		...
		$this->pageTitle = 'View Ommu Zone Provinces';
		$this->pageDescription = '';
		$this->pageMeta = $setting->meta_keyword;
		$this->render('/zone_province/front_view',array(
			'model'=>$model,
		));
		/*
		$this->render('/zone_province/admin_view',array(
			'model'=>$model,
		));
		*/
	}	
	...
	public function actionAdd() 
	{
		...
		if(isset($_POST['OmmuZoneProvince'])) {
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', 'OmmuZoneProvince success created.');
	...
	public function actionEdit($id) 
	{
		...
		if(isset($_POST['OmmuZoneProvince'])) {
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', 'OmmuZoneProvince success updated.');
					//$this->redirect(array('view','id'=>$model->province_id));
					$this->redirect(array('manage'));
				}
			}
	...
	public function actionHeadline($id) 
	{
		...
		if(Yii::app()->request->isPostRequest) {
			...
			$this->pageDescription = '';
			$this->pageMeta = '';
			$this->render('/zone_province/admin_headline');
		}
	}

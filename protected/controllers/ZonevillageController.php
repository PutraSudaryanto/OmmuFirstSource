...
class ZonevillageController extends Controller
{
	...
	public function actionIndex() 
	{
		...
		$setting = OmmuZoneVillage::model()->findByPk(1,array(
		...
		$dataProvider = new CActiveDataProvider('OmmuZoneVillage', array(
			'criteria'=>$criteria,
			'pagination'=>array(
				'pageSize'=>10,
			),
		...
		$this->pageTitle = 'Ommu Zone Villages';
		$this->pageDescription = $setting->meta_description;
		$this->pageMeta = $setting->meta_keyword;
		$this->render('/zone_village/front_index',array(
	...
	public function actionView($id) 
	{
		...
		$this->pageTitle = 'View Ommu Zone Villages';
		$this->pageDescription = '';
		$this->pageMeta = $setting->meta_keyword;
		$this->render('/zone_village/front_view',array(
			'model'=>$model,
		));
		/*
		$this->render('/zone_village/admin_view',array(
			'model'=>$model,
		));
		*/
	}	
	...
	public function actionAdd() 
	{
		...
		if(isset($_POST['OmmuZoneVillage'])) {
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', 'OmmuZoneVillage success created.');
	...
	public function actionEdit($id) 
	{
		...
		if(isset($_POST['OmmuZoneVillage'])) {
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', 'OmmuZoneVillage success updated.');
					//$this->redirect(array('view','id'=>$model->village_id));
					$this->redirect(array('manage'));
				}
			}
	...
	public function actionHeadline($id) 
	{
		...
		if(Yii::app()->request->isPostRequest) {
			...
			$this->pageDescription = '';
			$this->pageMeta = '';
			$this->render('/zone_village/admin_headline');
		}
	}

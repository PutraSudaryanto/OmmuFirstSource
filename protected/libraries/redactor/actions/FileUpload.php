...
class FileUpload extends CAction
{
	...
	public $createAttributePath=false;
	...
	public function run($attr)
	{
		...
		if ($file instanceof CUploadedFile) {
			...
			if($this->createAttributePath == true)
				$attributePath=$this->uploadPath.DIRECTORY_SEPARATOR.$name.DIRECTORY_SEPARATOR.$attribute;
			$fileName=time().'-'.$this->urlTitle(strstr($file->name,'.',true)).'.'.$file->getExtensionName();
			if($this->newFilename == true)
			...
			$attributeUrl=join('/', array($this->uploadUrl, $fileName));
			if($this->createAttributePath == true)
				$attributeUrl=join('/', array($this->uploadUrl, $name, $attribute, $fileName));
			$data = array(

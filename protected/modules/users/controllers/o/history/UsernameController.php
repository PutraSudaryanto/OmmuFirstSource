...
class UsernameController extends Controller
{
	...
	public function actionManage($user=null) 
	{
		...
		if($user != null) {
			...
			$pageTitle = Yii::t('phrase', 'History Usernames: $user_displayname level $level_name', array ('$user_displayname'=>$data->displayname,'$level_name'=>Phrase::trans($data->level->name)));
		}
	...
	public function actionDelete($id) 
	{
		...
		if(Yii::app()->request->isPostRequest) {
			...
			$this->pageTitle = Yii::t('phrase', 'Delete History Username: $user_displayname level $level_name', array('$user_displayname'=>$model->user->displayname,'$level_name'=>Phrase::trans($model->user->level->name)));
			$this->pageDescription = '';
			$this->pageMeta = '';
	...
	{
		$model = UserHistoryUsername::model()->findByPk($id);
		if($model===null)
			throw new CHttpException(404, Yii::t('phrase', 'The requested page does not exist.'));
		return $model;
	}
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='user-history-username-form') {

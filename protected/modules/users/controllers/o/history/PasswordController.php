...
class PasswordController extends Controller
{
		...
		if(Yii::app()->request->isPostRequest) {
			...
			if($model->delete()) {
				echo CJSON::encode(array(
					...
					'id' => 'partial-user-history-password',
					'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'History Password success deleted.').'</strong></div>',
				));
			}
			...
			$this->pageTitle = Yii::t('phrase', 'Delete History Password: $user_displayname level $level_name', array('$user_displayname'=>$model->user->displayname,'$level_name'=>Phrase::trans($model->user->level->name)));
			$this->pageDescription = '';
			$this->pageMeta = '';
	...
	{
		$model = UserHistoryPassword::model()->findByPk($id);
		if($model===null)
			throw new CHttpException(404, Yii::t('phrase', 'The requested page does not exist.'));
		return $model;
	}
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='user-history-password-form') {

<?php
	...
	$city = $model->office_city_id != 0 ? ', '.$model->city->city_name : '';
	$province = $model->office_province_id != 0 ? ', '.$model->province->province_name : '';
	$zipcode = $model->office_zipcode != '' ? ' '.$model->office_zipcode : '';
	$country = $model->office_country_id != 0 ? '<span class="highlight">'.$model->country->country_name.'</span>' : '';
?>

<?php
	...
	$city = $model->office_city != 0 ? ', '.$model->city->city_name : '';
	$province = $model->office_province != 0 ? ', '.$model->province->province_name : '';
	$zipcode = $model->office_zipcode != '' ? ' '.$model->office_zipcode : '';
	$country = $model->office_country != 0 ? '<span class="highlight">'.$model->country->country_name.'</span>' : '';
?>
